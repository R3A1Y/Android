...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        entrada = findViewById(R.id.main_edittext);
        entrada2 = findViewById(R.id.main_edittextmulti);
        button = findViewById(R.id.main_button);
        button2 = findViewById(R.id.main_button2);
    }
    public void grabar(View v) {
        String nomarchivo=entrada.getText().toString();
        nomarchivo=nomarchivo.replace('/','-');
        try {
            OutputStreamWriter archivo = new OutputStreamWriter(openFileOutput(
                    nomarchivo, Activity.MODE_PRIVATE));
            archivo.write(entrada2.getText().toString());
            archivo.flush();
            archivo.close();
        } catch (IOException e) {
        }
